...
const MusicPlayer = ({ nowPlaying }) => {
  ...
  const isLiked = likedSongs.some((song) => song.url === nowPlayingObj.url);
   ...
   async function addLike(){
    try{
      const data = await fetch('https://playlist-pal.duckdns.org/backend/api/v1/music/addToLiked' , { 
        method: 'post',
        headers: {
       ...
       async function removeLike(){
        ...
        try{
          const data = await fetch('https://playlist-pal.duckdns.org/backend/api/v1/music/removeFromLiked' , { 
            method: 'post',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${localStorage.getItem('db_token')}`
