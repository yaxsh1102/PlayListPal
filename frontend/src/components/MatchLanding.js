...
const MatchLanding = ({setsetSelectedOption}) => {
  ...
  async function matchHandleer(){   
    ...
    const response = await fetch('http://localhost:4000/api/v1/profile/getMatches', {
      ...
      body: JSON.stringify({playLists:ref.current['playLists'].checked , likedSongs:ref.current['likedSongs'].checked , radius:ref.current['radius'].value }),
    });
  ...
  return (
        ...
        <div className="w-full max-w-xs">
          ...
          <button className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded" onClick={matchHandleer}>
            Find My Match
          </button>
        </div>
      </div>
