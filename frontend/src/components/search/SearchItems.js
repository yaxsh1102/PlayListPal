...
const Searchitems = ({
  ...
  const options = [{ label: "Add to Queue" }, { label: "Add to Playlist" }];
  ...
  const toggleLike = () => {
    dispatch(
      liked
        ? removeFromLikedSongs(nowPlayingObj)
        : addToLikedSongs(nowPlayingObj)
    );
    if (liked) {
      removeLike();
    } else {
    ...
    dispatch(
      !liked
        ? sendToast("Added to Liked Songs")
        : sendToast("Removed from Liked Songs")
    );
  };
  ...
  const toggleCenterPopup = () => {
    setShowCenterPopup((prev) => !prev);
    ...
    <div className={`relative w-full flex`}>
      ...
      {showCenterPopup ? (
        <OptionPopup
          ...
          onClose={toggleCenterPopup}
          setShowCenterPopup={setShowCenterPopup}
          showPopup={showPopup}
