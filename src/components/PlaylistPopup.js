...
const PlayListPopup = ({ oldPlayList, setOldPlaylist, setShowAddToPlayList, setShowCenterPopup }) => {
  ...
  useEffect(() => {
    ...
    Object.keys(playlist).forEach(option => {
      status[option] = playlist[option].some(song => song.id === selectedSong.id);
    });
      ...
      {oldPlayList ? (
        <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 py-4 shadow-lg rounded-lg z-50 h-[20rem] w-[16rem] flex flex-col items-stretch bg-[#191919] text-slate-200">
          ...
          <div className="flex flex-col justify-center">
            <div className='flex w-full gap-x-2 px-6 border-b-[2px] py-3 border-b-slate-600 hover:bg-gray-800 ' onClick={() => setOldPlaylist(false)}>
              <p>
        ...
        >
          ...
          <button className='sm:w-[60%] w-[45%] h-[3rem] mx-auto bg-[#232323] my-auto hover:bg-[#3f3f3f] rounded-sm' type='submit'>
            Save
          </button>
        </form>
      )}
